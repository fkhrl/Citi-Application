...
class db_class {
    ...
    public function open() {
        $con=mysqli_connect("localhost", "root", "", "citi_application");
        return $con;
    }

    public function pdocon() {
        $db=new PDO("mysql:host=localhost;dbname=citi_application;", "root", "");
    ...
    function baseUrl($suffix='') {
        ...
        $web_root=$protocol . $_SERVER['HTTP_HOST'] . "/" . "fakhrul/citi-website/cms-admin/";
        $suffix=ltrim($suffix, '/');
    ...
    function LbaseUrl($suffix='') {
        $protocol=strpos($_SERVER['SERVER_SIGNATURE'], '443') !== false ? 'https://' : 'http://';
        $web_root=$protocol . $_SERVER['HTTP_HOST'] . "/" . "fakhrul/citi-website/";
        $suffix=ltrim($suffix, '/');
        return $web_root . trim($suffix);
    }
